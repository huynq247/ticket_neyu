...
from app.providers.telegram import telegram_provider
...
def send_telegram_notification(self, notification_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Get recipient chat ID from User Service (TODO: implement API call)
        # For now, we'll assume recipient_id is the chat ID
        ...
        # Render template
        message_text = template_engine.render_template(template_name, template_data)
        if not message_text:
        ...
        import asyncio
        result = asyncio.run(telegram_provider.send_message(
            chat_id=chat_id,
            text=message_text
        ))
