...
def migrate_permissions_direct_db():
    ...
    # Connection parameters - adjust as needed for your environment
    ...
    try:
        ...
        if os.path.exists(env_file):
            ...
            db_name = conn_params.get('POSTGRES_DB', 'postgres')  # Use 'postgres' as default database
            db_user = conn_params.get('POSTGRES_USER', 'admin')   # Use 'admin' as default user
            ...
            db_host = 'localhost'
            db_port = '5432'
            db_name = 'postgres'  # Default database in PostgreSQL
            db_user = 'admin'     # Try 'admin' instead of 'postgres'
            db_password = 'admin' # Default password
    ...
    if 'y' in input(f"Use connection: {db_user}@{db_host}:{db_port}/{db_name}? (y/n): ").lower():
        pass
    else:
        db_host = input(f"Database host [{db_host}]: ") or db_host
        db_port = input(f"Database port [{db_port}]: ") or db_port
        db_name = input(f"Database name [{db_name}]: ") or db_name
    ...
    try:
        ...
        if not db_exists:
            ...
            print("Connecting to the new ticket_db database...")
            conn = psycopg2.connect(
                host=db_host,
                port=db_port,
                database="ticket_db",
            ...
            # Close connection and reconnect to ticket_db
            cur.close()
            conn.close()
            
            print("Connecting to ticket_db database...")
            conn = psycopg2.connect(
                host=db_host,
                port=db_port,
                database="ticket_db",
