...
def create_test_data():
    ...
    try:
        ...
        # Check what values are valid for the role field
        cursor.execute("SELECT unnest(enum_range(NULL::user_role_enum))")
        valid_roles = [r[0] for r in cursor.fetchall()]
        print(f"Valid roles in the database: {valid_roles}")
        
        # Insert test users with appropriate role values
        users = [
            {
                ...
                "role": "admin" if "admin" in valid_roles else valid_roles[0],  # Use admin if available, otherwise first valid role
                "is_active": True
            },
            {
                ...
                "role": "manager" if "manager" in valid_roles else valid_roles[0],
                "is_active": True
            },
            {
                ...
                "role": "agent" if "agent" in valid_roles else valid_roles[0],
                "is_active": True
            },
            {
                ...
                "role": "user" if "user" in valid_roles else valid_roles[0],
                "is_active": True
            },
            {
        ...
        for user_data in users:
            ...
            if result:
                ...
                cursor.execute(
                    "INSERT INTO users (email, username, hashed_password, full_name, role, is_active) VALUES (%s, %s, %s, %s, %s, %s) RETURNING id",
                    (
                        user_data["email"], 
                        user_data["username"], 
